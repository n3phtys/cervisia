...
lazy_static! {
    ...
    pub static ref GLOBAL_QUICKMENU: Mutex<QuickmenuGtkComponents> = Mutex::new(build_quickmenu());
    pub static ref GLOBAL_USERWINDOW: Mutex<UserWindowGtkComponents> = Mutex::new(build_from_glade());

    pub static ref ADD_OR_UNDO_PURCHASE : Mutex<(Sender<Purchase>, Sender<Purchase>)> = Mutex::new(build_purchase_debouncer());
